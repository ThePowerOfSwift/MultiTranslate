//
//  SupportedLanguages.swift
//  MultiTranslate
//
//  Created by Keishin CHOU on 2020/02/05.
//  Copyright © 2020 Keishin CHOU. All rights reserved.
//

import Foundation

struct SupportedLanguages {
    static let gcpLanguageList = [
        "Afrikaans".localized(),
        "Albanian".localized(),
        "Amharic".localized(),
        "Arabic".localized(),
        "Armenian".localized(),
        "Azerbaijani".localized(),
        "Basque".localized(),
        "Belarusian".localized(),
        "Bengali".localized(),
        "Bosnian".localized(),
        "Bulgarian".localized(),
        "Catalan".localized(),
        "Cebuano".localized(),
        "Chinese (Mandarin)".localized(),
        "Chinese (HK)".localized(),
        "Corsican".localized(),
        "Croatian".localized(),
        "Czech".localized(),
        "Danish".localized(),
        "Dutch".localized(),
        "English".localized(),
        "Esperanto".localized(),
        "Estonian".localized(),
        "Finnish".localized(),
        "French".localized(),
        "Frisian".localized(),
        "Galician".localized(),
        "Georgian".localized(),
        "German".localized(),
        "Greek".localized(),
        "Gujarati".localized(),
        "Haitian Creole".localized(),
        "Hausa".localized(),
        "Hawaiian".localized(),
        "Hebrew".localized(),
        "Hindi".localized(),
        "Hmong".localized(),
        "Hungarian".localized(),
        "Icelandic".localized(),
        "Igbo".localized(),
        "Indonesian".localized(),
        "Irish".localized(),
        "Italian".localized(),
        "Japanese".localized(),
        "Javanese".localized(),
        "Kannada".localized(),
        "Kazakh".localized(),
        "Khmer".localized(),
        "Korean".localized(),
        "Kurdish".localized(),
        "Kyrgyz".localized(),
        "Lao".localized(),
        "Latin".localized(),
        "Latvian".localized(),
        "Lithuanian".localized(),
        "Luxembourgish".localized(),
        "Macedonian".localized(),
        "Malagasy".localized(),
        "Malay".localized(),
        "Malayalam".localized(),
        "Maltese".localized(),
        "Maori".localized(),
        "Marathi".localized(),
        "Mongolian".localized(),
        "Myanmar (Burmese)".localized(),
        "Nepali".localized(),
        "Norwegian".localized(),
        "Nyanja (Chichewa)".localized(),
        "Pashto".localized(),
        "Persian".localized(),
        "Polish".localized(),
        "Portuguese".localized(),
        "Punjabi".localized(),
        "Romanian".localized(),
        "Russian".localized(),
        "Samoan".localized(),
        "Scots Gaelic".localized(),
        "Serbian".localized(),
        "Sesotho".localized(),
        "Shona".localized(),
        "Sindhi".localized(),
        "Sinhala (Sinhalese)".localized(),
        "Slovak".localized(),
        "Slovenian".localized(),
        "Somali".localized(),
        "Spanish".localized(),
        "Sundanese".localized(),
        "Swahili".localized(),
        "Swedish".localized(),
        "Tagalog (Filipino)".localized(),
        "Tajik".localized(),
        "Tamil".localized(),
        "Telugu".localized(),
        "Thai".localized(),
        "Turkish".localized(),
        "Ukrainian".localized(),
        "Urdu".localized(),
        "Uzbek".localized(),
        "Vietnamese".localized(),
        "Welsh".localized(),
        "Xhosa".localized(),
        "Yiddish".localized(),
        "Yoruba".localized(),
        "Zulu".localized(),
    ]
    
    static let gcpLanguageListOrigine = [
        "Afrikaans",
        "Albanian",
        "Amharic",
        "Arabic",
        "Armenian",
        "Azerbaijani",
        "Basque",
        "Belarusian",
        "Bengali",
        "Bosnian",
        "Bulgarian",
        "Catalan",
        "Cebuano",
        "Chinese (Mandarin)",
        "Chinese (HK)",
        "Corsican",
        "Croatian",
        "Czech",
        "Danish",
        "Dutch",
        "English",
        "Esperanto",
        "Estonian",
        "Finnish",
        "French",
        "Frisian",
        "Galician",
        "Georgian",
        "German",
        "Greek",
        "Gujarati",
        "Haitian Creole",
        "Hausa",
        "Hawaiian",
        "Hebrew",
        "Hindi",
        "Hmong",
        "Hungarian",
        "Icelandic",
        "Igbo",
        "Indonesian",
        "Irish",
        "Italian",
        "Japanese",
        "Javanese",
        "Kannada",
        "Kazakh",
        "Khmer",
        "Korean",
        "Kurdish",
        "Kyrgyz",
        "Lao",
        "Latin",
        "Latvian",
        "Lithuanian",
        "Luxembourgish",
        "Macedonian",
        "Malagasy",
        "Malay",
        "Malayalam",
        "Maltese",
        "Maori",
        "Marathi",
        "Mongolian",
        "Myanmar (Burmese)",
        "Nepali",
        "Norwegian",
        "Nyanja (Chichewa)",
        "Pashto",
        "Persian",
        "Polish",
        "Portuguese",
        "Punjabi",
        "Romanian",
        "Russian",
        "Samoan",
        "Scots Gaelic",
        "Serbian",
        "Sesotho",
        "Shona",
        "Sindhi",
        "Sinhala (Sinhalese)",
        "Slovak",
        "Slovenian",
        "Somali",
        "Spanish",
        "Sundanese",
        "Swahili",
        "Swedish",
        "Tagalog (Filipino)",
        "Tajik",
        "Tamil",
        "Telugu",
        "Thai",
        "Turkish",
        "Ukrainian",
        "Urdu",
        "Uzbek",
        "Vietnamese",
        "Welsh",
        "Xhosa",
        "Yiddish",
        "Yoruba",
        "Zulu"
    ]
    
    static let gcpLanguageCode = [
        "af","sq","am","ar","hy","az","eu","be","bn","bs","bg","ca","ceb","zh-CN","zh-TW","co","hr","cs","da","nl","en","eo","et","fi","fr","fy","gl","ka","de",
        "el","gu","ht","ha","haw","he","hi","hmn","hu","is","ig","id","ga","it","ja","jv","kn","kk","km","ko","ku","ky","lo","la","lv","lt","lb","mk","mg","ms",
        "ml","mt","mi","mr","mn","my","ne","no","ny","ps","fa","pl","pt","pa","ro","ru","sm","gd","sr","st","sn","sd","si","sk","sl","so","es","su","sw","sv",
        "tl","tg","ta","te","th","tr","uk","ur","uz","vi","cy","xh","yi","yo","zu"
    ]
    
    static let speechRecognizerSupportedLocaleIdentifier = [
        // Use for speech recognizing
        "ar-SA", "ca-ES", "cs-CZ", "da-DK", "de-DE", "de-AT", "de-CH", "el-GR", "en-US", "en-AU", "en-CA", "en-GB", "en-IE", "en-IN", "en-NZ", "en-PH",
        "en-SA", "en-SG",  "en-ZA", "es-ES", "es-419", "es-CL", "es-CO", "es-MX", "es-US", "fi-FI", "fr-FR", "fr-BE", "fr-CA", "fr-CH", "he-IL", "hi-IN",
        "hr-HR", "hu-HU", "id-ID", "it-IT", "it-CH", "ja-JP", "ko-KR", "ms-MY", "nb-NO", "nl-NL", "nl-BE", "pl-PL", "pt-PT", "pt-BR",
        "ro-RO", "ru-RU", "sk-SK", "sv-SE", "th-TH", "tr-TR", "uk-UA", "vi-VN", "zh-CN", "zh-HK"
//        "ar-SA", "ca-ES", "cs-CZ", "da-DK", "de-AT", "de-CH", "de-DE", "el-GR", "en-AE", "en-AU", "en-CA", "en-GB", "en-ID", "en-IE", "en-IN", "en-NZ", "en-PH",
//        "en-SA", "en-SG", "en-US", "en-ZA", "es-419", "es-CL", "es-CO", "es-ES", "es-MX", "es-US", "fi-FI", "fr-BE", "fr-CA", "fr-CH", "fr-FR", "he-IL", "hi-IN",
//        "hi-IN-translit", "hi-Latn", "hr-HR", "hu-HU", "id-ID", "it-CH", "it-IT", "ja-JP", "ko-KR", "ms-MY", "nb-NO", "nl-BE", "nl-NL", "pl-PL", "pt-BR", "pt-PT",
//        "ro-RO", "ru-RU", "sk-SK", "sv-SE", "th-TH", "tr-TR", "uk-UA", "vi-VN", "wuu-CN", "yue-CN", "zh-CN", "zh-HK", "zh-TW"
    ]
    
    static let speechRecognizerSupportedLocale = [
        // Use for display in LanguagePickerViewController
        "Arabic (Saudi Arabia)".localized(),
        "Catalan (Spain)".localized(),
        "Czech (Czech)".localized(),
        "Danish".localized(),
        "German (Germany)".localized(),
        "German (Austria)".localized(),
        "German (Switzerland)".localized(),
        "Greek".localized(),
        "English (United States)".localized(),
        "English (Australia)".localized(),
        "English (Canada)".localized(),
        "English (United Kingdom)".localized(),
        "English (Ireland)".localized(),
        "English (India)".localized(),
        "English (New Zealand)".localized(),
        "English (Philippines)".localized(),
        "English (Saudi Arabia)".localized(),
        "English (Singapore)".localized(),
        "English (South Africa)".localized(),
        "Spanish (Spain)".localized(),
        "Spanish (Latin America & Caribbean)".localized(),
        "Spanish (Chile)".localized(),
        "Spanish (Colombia)".localized(),
        "Spanish (Mexico)".localized(),
        "Spanish (United States)".localized(),
        "Finnish (Finland)".localized(),
        "French (France)".localized(),
        "French (Belgium)".localized(),
        "French (Canada)".localized(),
        "French (Switzerland)".localized(),
        "Hebrew (Israel)".localized(),
        "Hindi (India)".localized(),
        "Croatian (Croatia)".localized(),
        "Hungarian (Hungary)".localized(),
        "Indonesian (Indonesia)".localized(),
        "Italian (Italy)".localized(),
        "Italian (Switzerland)".localized(),
        "Japanese (Japan)".localized(),
        "Korean (Korea)".localized(),
        "Malay (Malaysia)".localized(),
        "Norwegian (Norway)".localized(),
        "Dutch (Netherlands)".localized(),
        "Dutch (Belgium)".localized(),
        "Polish (Poland)".localized(),
        "Portuguese (Portugal)".localized(),
        "Portuguese (Brazil)".localized(),
        "Romanian (Romania)".localized(),
        "Russian (Russia)".localized(),
        "Slovak (Slovakia)".localized(),
        "Swedish (Sweden)".localized(),
        "Thai (Thailand)".localized(),
        "Turkish (Turkey)".localized(),
        "Ukrainian (Ukraine)".localized(),
        "Vietnamese (Vietnam)".localized(),
        "Chinese (China)".localized(),
        "Chinese (Hongkong)".localized(),
    ]
    
    static let speechRecognizerSupportedLocaleOrigine = [
        // Use for display in LanguagePickerViewController
        "Arabic (Saudi Arabia)", "Catalan (Spain)", "Czech (Czech)", "Danish", "German (Germany)", "German (Austria)", "German (Switzerland)", "Greek", "English (United States)", "English (Australia)",
        "English (Canada)", "English (United Kingdom)", "English (Ireland)", "English (India)", "English (New Zealand)", "English (Philippines)",
        "English (Saudi Arabia)", "English (Singapore)", "English (South Africa)", "Spanish (Spain)", "Spanish (Latin America & Caribbean)", "Spanish (Chile)", "Spanish (Colombia)",
        "Spanish (Mexico)", "Spanish (United States)", "Finnish (Finland)", "French (France)", "French (Belgium)", "French (Canada)", "French (Switzerland)",
        "Hebrew (Israel)", "Hindi (India)", "Croatian (Croatia)", "Hungarian (Hungary)", "Indonesian (Indonesia)", "Italian (Italy)", "Italian (Switzerland)",
        "Japanese (Japan)", "Korean (Korea)", "Malay (Malaysia)", "Norwegian (Norway)", "Dutch (Netherlands)", "Dutch (Belgium)", "Polish (Poland)",
        "Portuguese (Portugal)", "Portuguese (Brazil)", "Romanian (Romania)", "Russian (Russia)", "Slovak (Slovakia)", "Swedish (Sweden)", "Thai (Thailand)",
        "Turkish (Turkey)", "Ukrainian (Ukraine)", "Vietnamese (Vietnam)", "Chinese (China)", "Chinese (Hongkong)"
    ]
    
    static let speechRecognizerSupportedLanguage = [
        // Use for display in VoiceTranslationViewController
        "Arabic", "Catalan", "Czech", "Danish", "German", "German", "German", "Greek", "English",
        "English", "English", "English", "English", "English", "English",
        "English", "English", "English", "English", "Spanish", "Spanish", "Spanish",
        "Spanish", "Spanish", "Spanish", "Finnish", "French", "French", "French",
        "French", "Hebrew", "Hindi", "Croatian", "Hungarian", "Indonesian", "Italian",
        "Italian", "Japanese", "Korean", "Malay", "Norwegian", "Dutch", "Dutch", "Polish",
        "Portuguese", "Portuguese", "Romanian", "Russian", "Slovak", "Swedish", "Thai",
        "Turkish", "Ukrainian", "Vietnamese", "Chinese (Mandarin)", "Chinese (HK)"
    ]
    
    static let speechRecognizerSupportedLanguageCode = [
        //Use for translating
        "ar", "ca", "cs", "da", "de", "de", "de", "el", "en", "en", "en", "en", "en", "en", "en",
        "en", "en", "en", "en", "es", "es", "es", "es", "es", "es", "fi", "fr", "fr", "fr", "fr", "he", "hi",
        "hr", "hu", "id", "it", "it", "ja", "ko", "ms", "nb", "nl", "nl", "pl", "pt", "pt",
        "ro", "ru", "sk", "sv", "th", "tr", "uk", "vi", "zh", "zh"
    ]
    
    static let visionRecognizerSupportedLanguage = [
        "German".localized(),
        "English".localized(),
        "Spanish".localized(),
        "Finnish".localized(),
        "French".localized(),
        "Italian".localized(),
    ]
    
    static let visionRecognizerSupportedLanguageOrigine = [
        "German", "English", "Spanish", "Finnish", "French", "Italian"
    ]
    
    static let fbSupportedLanguage = ["Afrikaans", "Arabic", "Belarusian", "Bulgarian", "Bengali", "Catalan", "Czech", "Welsh", "Danish", "German", "Greek", "English", "Esperanto",
                                      "Spanish", "Estonian", "Persian", "Finnish", "French", "Irish", "Galician", "Gujarati", "Hebrew", "Hindi", "Croatian", "Haitian Creole",
                                      "Hungarian", "Indonesian", "Icelandic", "Italian", "Japanese", "Georgian", "Kannada", "Korean", "Lithuanian", "Latvian", "Macedonian",
                                      "Marathi", "Malay", "Maltese", "Dutch", "Norwegian", "Polish", "Portuguese", "Romanian", "Russian", "Slovak", "Slovenian", "Albanian",
                                      "Swedish", "Swahili", "Tamil", "Telugu", "Thai", "Tagalog (Filipino)", "Turkish", "Ukrainian", "Urdu", "Vietnamese", "Chinese (Mandarin)"]
    
    static let fbSupportedLanguageCode = ["af", "ar", "be", "bg", "bn", "ca", "cs", "cy", "da", "de", "el", "en", "eo", "es", "et", "fa", "fi", "fr", "ga", "gl", "gu", "he", "hi",
                                          "hr", "ht", "hu", "id", "is", "it", "ja", "ka", "kn", "ko", "lt", "lv", "mk", "mr", "ms", "mt", "nl", "no", "pl", "pt", "ro", "ru", "sk",
                                          "sl", "sq", "sv", "sw", "ta", "te", "th", "tl", "tr", "uk", "ur", "vi", "zh"]
    
}
